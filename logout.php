...
if (isset($_GET['confirm']) && $_GET['confirm'] == 'yes') {
    ...
    // Set flash message in current session before destroying it
    $_SESSION['flash_message'] = "You have been successfully logged out, " . htmlspecialchars($username) . ".";
    ...
    // Start a brand new empty session so the flash message can be carried over
    session_start();

    // Move the flash message to the new session (it survived regenerate but we ensure it's there)
    // Note: after regenerate_id(true), the old data is still available until destroy(),
    // but we already unset everything, so we need to set it again if needed.
    // Since we set it before unset, and regenerate keeps data, it's still there.
    // But to be 100% safe, we set it again in the new session:
    if (isset($_SESSION['flash_message'])) {
        // It's already there due to regenerate keeping data before destroy
    } else {
        $_SESSION['flash_message'] = "You have been successfully logged out" . ($username !== 'User' ? ", " . htmlspecialchars($username) : "") . ".";
    }

    // Redirect to login without any parameters in URL
